<template>
  ...
  >
    ...
    <!-- <el-form-item
            label="富文本"
            prop="editor"
            :rules="[{ required: true, message: '富文本不能为空' }]"
          >
            <tinymce :value="proxyForm.editor" paramKey="editor" @input="crudTinymce"  height="300" />
          </el-form-item> -->
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      init: {
        ...
        height: Number(this.height), // 注：引入autoresize插件时，此属性失效
        placeholder: '在这里输入文字',
        ...
        images_upload_handler: (blobInfo, success, failure) => {
          if (blobInfo.blob().size / 1024 / 1024 > 2) {
            ...
            params.append('pathName', 'image');
            params.append('is_front', 0);
            // let config = {
            //   headers: {
  ...
  watch: {
    ...
    contentValue(newValue) {
      this.$emit('input', this.paramKey, newValue);
    }
  },
  created() {},
  mounted() {
