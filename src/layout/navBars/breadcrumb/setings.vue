  ...
  <div class="layout-breadcrumb-seting">
    ...
    >
      <el-scrollbar class="layout-breadcrumb-seting-bar">
        ...
        <div class="layout-breadcrumb-seting-bar-flex mt14">
          <div class="layout-breadcrumb-seting-bar-flex-label">
            {{ $t('layout.twoIsMenuBarColorHighlight') }}
          </div>
          <div class="layout-breadcrumb-seting-bar-flex-value">
            <el-switch
        ...
        <div class="layout-breadcrumb-seting-bar-flex mt14">
          ...
          <div class="layout-breadcrumb-seting-bar-flex-value">
            <el-input
              v-model.trim="getThemeConfig.wartermarkText"
              style="width: 90px"
        ...
        <div class="copy-config">
          ...
          <el-button
            ...
            ><el-icon><copy-document /></el-icon>{{ $t('layout.copyText') }}
          </el-button>
        </div>
      </el-scrollbar>
    </el-drawer>
...
export default defineComponent({
  ...
  emits: ['openOrCloseSortable', 'getBreadcrumbIndexSetFilterRoutes'],
  setup() {
    const { t } = useI18n();
    ...
    const setPropertyFun = (color: string, targetVal: any) => {
      ...
      for (let i = 1; i <= 9; i++) {
        document.documentElement.style.setProperty(
          `${color}-light-${i}`,
          getLightColor(targetVal, i / 10) as any
        );
      }
    };
    // 2、菜单 / 顶栏
    const onBgColorPickerChange = (bg: any) => {
      document.documentElement.style.setProperty(
        `--bg-${bg}`,
        (getThemeConfig.value as any)[bg] as any
      );
    ...
    const setGraduaFun = (el: string, bool: boolean, color: string) => {
      nextTick(() => {
        ...
        setLocalThemeConfig();
        const elNavbars: any = document.querySelector(
          '.layout-navbars-breadcrumb-index'
        );
        const elAside: any = document.querySelector(
          '.layout-container .el-aside'
        );
        const elColumns: any = document.querySelector(
          '.layout-container .layout-columns-aside'
        );
    ...
    const onCopyConfigClick = (target: any) => {
      const copyThemeConfig = getLocal('themeConfig');
      ...
      clipboard.on('success', () => {
        getThemeConfig.value.isDrawer = false;
        ElMessage({ type: 'success', message: t('layout.copyTextSuccess') });
        clipboard.destroy();
      });
      clipboard.on('error', () => {
        ElMessage({ type: 'error', message: t('layout.copyTextError') });
        clipboard.destroy();
      });
    };
    onMounted(() => {
      nextTick(() => {
        ...
        window.addEventListener('load', () => {
          ...
          setTimeout(() => {
            // 顶栏背景渐变
            if (
              getLocal('navbarsBgStyle') &&
              getThemeConfig.value.isTopBarColorGradual
            ) {
              const breadcrumbIndexEl: any = document.querySelector(
                '.layout-navbars-breadcrumb-index'
              );
            ...
            // 菜单背景渐变
            if (
              getLocal('asideBgStyle') &&
              getThemeConfig.value.isMenuBarColorGradual
            ) {
              const asideEl: any = document.querySelector(
                '.layout-container .el-aside'
              );
            ...
            // 菜单字体背景高亮
            if (
              getLocal('menuBarHighlightId') &&
              getThemeConfig.value.isMenuBarColorHighlight
            ) {
              const els = document.querySelector('.el-menu-item.is-active');
              if (!els) return false;
