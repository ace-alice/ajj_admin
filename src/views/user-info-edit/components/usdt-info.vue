  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        :label="$t('table.usdt_address')"
        align="center"
      />
      <el-table-column :label="$t('table.usdt_type')" align="center">
        <template #default="scope">
          <span>USDT</span>
        </template>
      </el-table-column>
      <el-table-column prop="protocol_code" :label="$t('table.usdt_protocol')" align="center" />
      <el-table-column
        :label="$t('table.usdt_account_status')"
        align="center"
      >
        <template #default="scope">
          <span>绑定</span>
      ...
      >
        <template #default="scope">
          <el-button type="text" @click="handleDelete(scope.row)">{{
            scope.row.status === 1 ? $t('table.turn_off') : $t('table.turn_on')
          }}</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default defineComponent({
  ...
  setup() {
    ...
    async function handleDelete(row: any) {
      ...
      const { code, msg } = await (<any>userBlockChainChange(params));
      if (code === 1) {
        toQuery();
      } else {
        ElMessage.error(msg);
