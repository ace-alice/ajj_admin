    ...
    <crudOperation title="router.recharge-list" labelWidth="120px">
      <template #query>
        ...
        <el-form-item label="订单状态">
          <el-select
            v-model="crudQuery.order_status_system"
            popper-class="user-select"
            clearable
            style="width: 200px"
          >
        ...
        <el-form-item :label="$t('table.verify_time' /**审核时间 */)">
          <dateTimePickerQuery
            @setTime="crudSetQueryKey"
            startKey="verify_start_time"
            endKey="verify_end_time"
            :propTime="[crudQuery.verify_start_time, crudQuery.verify_end_time]"
          />
        </el-form-item>
      </template>
      <template #table>
        ...
        >
          ...
          >
            <template #default="scope">
              <span>{{
                getLabel(scope.row.order_status_system, order_status_options)
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
...
export default defineComponent({
  ...
  setup() {
    ...
    console.log('moneyChangeTypeList', cashManualTypeList);
    const dict = reactive({
