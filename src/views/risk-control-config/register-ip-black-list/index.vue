<template>
  <div class="main-container">
    <crudOperation title="router.register-ip-black-list">
      <template #query>
        <el-form-item :label="$t('table.IP')">
          <el-input v-model.trim="crudQuery.ip" clearable></el-input>
        </el-form-item>
        <el-form-item :label="$t('table.ip_local')">
          <el-input v-model.trim="crudQuery.ip_local" clearable></el-input>
        </el-form-item>
      ...
      <template #form>
        ...
        >
          <el-form-item
            ...
            :rules="[
              {
                required: true,
                message: $t('table.IP') + $t('table.not_empty')
              }
            ]"
          >
            <el-input
              v-model.trim="proxyForm.ip"
          ...
          <el-form-item
            ...
            :rules="[
              {
                required: true,
                message: $t('table.ip_remark') + $t('table.not_empty')
              }
            ]"
          >
            <el-input
              v-model.trim="proxyForm.ip_remark"
        ...
        <el-card
          class="box-card"
          v-if="
            crudFindRole.only(otherPermission.getRegisterList) &&
            crudStatus.edit == 1
          "
        >
          <template #header>
...
<script lang="ts">
import Api from '@/api/risk-control-config/register-ip-black-list';
...
const defaultCrudOptions: CrudOptionsType = {
  ...
  crudTag: 'risk:register:ip:'
};

export default defineComponent({
  name: 'register-ip-black-list',
  ...
  setup() {
    ...
    crud.crudHook.beforeToEdit = (F: any) => {
      ...
      if (crud.crudFindRole.only(otherPermission.getRegisterList)) {
        getUserList();
      }
      return true;
    };
    ...
    const otherPermission = {
      //是否可以需改需要权限验证
      getRegisterList: 'risk:register:ip:getRegisterList',
      forbid: 'risk:register:ip:forbid'
    };
