...
export default {
  ...
  emits: ['onTagsViewRefreshRouterView'],
  setup() {
    ...
    const addBrowserSetSession = (tagsViewList: Array<any>) => {
    ...
    const onCurrentContextmenuClick = (data: any) => {
      const { id, path } = data;
      const currentTag = state.tagsViewList.find((v: any) => v.path === path);
      switch (id) {
        case 0:
    ...
    onBeforeMount(() => {
      ...
      proxy.mittBus.on('onCurrentContextmenuClick', (data: any) => {
